...
//3.1 Scheduler
struct proc *choose_next_process(void); // <-- Move this here
...
getptable(int nproc, uint64 buffer)
{
  ...
  for(p = proc; p < &proc[NPROC] && count < nproc; p++){
    acquire(&p->lock);
    // Remove the "if(p->state != UNUSED)" check to include all processes
    info.pid = p->pid;
...
freeproc(struct proc *p)
{
  ...
  p->killed = 0;
  p->xstate = 0;
  // p->creation_time = 0; // reset creation_time
  // p->run_time = 0;      // reset run_time
  p->state = UNUSED;
}
...
void //1.1 Sleep for a number of milliseconds
